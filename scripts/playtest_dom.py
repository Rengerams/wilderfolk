...
with sync_playwright() as p:
    ...
    dom = page.evaluate(
        ...
        " i, w: c.width, h: c.height, cls: c.className || '',"
        " style: (c.getAttribute('style') || '').slice(0, 140),"
        " parent: c.parentElement ? c.parentElement.tagName + (c.parentElement.id ? '#' + c.parentElement.id : '') : '' }))"
    )
    print("canvas dom:", dom)
