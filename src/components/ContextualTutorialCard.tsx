      ...
      <div className="rounded-xl border border-amber-500/35 bg-stone-900/95 p-3 shadow-2xl backdrop-blur-sm">
        <div className="mb-1 flex items-start justify-between gap-2">
          <div className="flex min-w-0 items-start gap-2">
            ...
            <div className="min-w-0">
              <p className="text-xs font-bold uppercase tracking-wide text-amber-400">New</p>
          ...
          <button
            ...
            className="shrink-0 rounded px-1.5 py-0.5 text-xs text-stone-400 hover:bg-stone-800 hover:text-stone-200"
            aria-label="Dismiss tip"
          >
            ✕
          </button>
        </div>
        <p className="mb-2 text-[13px] leading-relaxed text-stone-300">{tip.detail}</p>
        <div className="flex gap-2">
          {action && onAction && (
            <button
              type="button"
              onClick={() => onAction(action)}
              className="flex-1 rounded-lg bg-amber-700/80 px-2 py-1.5 text-xs font-bold text-amber-50 hover:bg-amber-600/90"
            >
              {action.label} →
            </button>
          )}
          <button
            ...
            className={`rounded-lg bg-stone-700 px-2 py-1.5 text-xs font-semibold text-stone-200 hover:bg-stone-600 ${action ? '' : 'w-full'}`}
          >
            Got it
          </button>
        </div>
        {onDisableAll && (
          <button
            ...
            className="mt-2 w-full text-center text-[11px] font-semibold text-stone-400 hover:text-stone-300"
          >
