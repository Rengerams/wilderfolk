...
function OutcomeBadge({ tier, kind }: { tier: RaidOutcomeTier | CounterRaidTier; kind: 'defend' | 'counter' }) {
  ...
  return (
    <span className={`inline-block rounded px-1.5 py-0.5 text-[10px] font-bold ${toneClass}`} title={meta.hint}>
      {meta.label}
    </span>
  );
}
...
export default function CombatPreviewPanel({
  ...
  return (
    <div className={`rounded-lg border border-stone-600/50 bg-stone-900/60 ${compact ? 'p-2' : 'p-2.5'}`}>
      <h4 className="mb-1.5 text-[11px] font-bold uppercase tracking-wider text-stone-400">
        {title ?? '⚔️ Raid readiness forecast'}
      </h4>
      {!compact && (
        <p className="mb-2 rounded bg-stone-800/80 px-2 py-1 text-[10px] leading-relaxed text-stone-300">
          {RAID_PREPARATION_HINT}
        </p>
      )}

      {preview.distanceLabel && (
        <div className="mb-2 flex flex-wrap items-center justify-between gap-1 rounded bg-stone-800/80 px-2 py-1 text-[11px]">
          <span className="text-stone-400">📍 Distance to camp</span>
          <strong className="text-cyan-300">{preview.distanceLabel}</strong>
        </div>
      )}

      {preview.armamentLabel && (
        <p className="mb-1.5 rounded bg-emerald-950/30 px-2 py-1 text-[10px] font-semibold text-emerald-300/90">
          🏹 {preview.armamentLabel}
        </p>
      )}
      ...
      <div className="mb-2 grid grid-cols-2 gap-1.5 text-[11px]">
        <div className="rounded bg-stone-800/80 px-2 py-1">
          <div className="text-stone-400">Your militia</div>
          <strong className="text-lg text-emerald-300">{preview.militiaStrength}</strong>
          <div className="text-stone-400">{preview.militiaCount} adults</div>
        </div>
        <div className="rounded bg-stone-800/80 px-2 py-1">
          <div className="text-stone-400">Rival raid power</div>
          <strong className="text-lg text-rose-300">{preview.rivalStrength ?? '—'}</strong>
          {preview.defendRatio != null && (
            <div className="text-stone-400">Ratio {ratioPct(preview.defendRatio)}</div>
          )}
        </div>
      </div>
      ...
      {preview.rivalStrength != null && (
        <div className="space-y-1 border-t border-stone-700/60 pt-2 text-[11px]">
          <p className="mb-1 text-[10px] font-semibold text-stone-300">If they raid your village:</p>
          <div className="flex flex-wrap items-center justify-between gap-1">
            ...
            {preview.hasSpears
              ? (preview.defendOutcome
                  ...
                  : (
                    <span
                      className="inline-block rounded border border-stone-600/40 bg-stone-800/80 px-1.5 py-0.5 text-[10px] font-bold text-stone-400"
          ...
          <div className="flex flex-wrap items-center justify-between gap-1">
            ...
            <span className="text-[10px] text-stone-400">No fight — tribute food, they leave</span>
          </div>
          {preview.incomingPayoffFood != null
            && preview.outgoingRaidFoodCost != null
            && preview.incomingPayoffFood < preview.outgoingRaidFoodCost && (
            <p className="rounded bg-cyan-950/40 px-1.5 py-1 text-[10px] font-semibold text-cyan-300">
          ...
          <p className="mt-1 text-[10px] italic text-stone-600">{DEFENSE_RATIO_HINT}</p>
          <p className="text-[10px] italic text-stone-600">{MILITIA_TIER_HINT}</p>
          {showOutgoingRaid && (
            <>
              <p className="mb-0.5 mt-1.5 text-[10px] font-semibold text-stone-300">{outgoingHeading}</p>
              {preview.counterRaidRivalStrength != null && (
                <p className="mb-1 text-[10px] text-stone-300">
              ...
              <p className="mt-1 text-[10px] italic text-stone-600">{COUNTER_RAID_RATIO_HINT}</p>
            </>
          )}
        </div>
      )}
      ...
      {!preview.hasSpears && (
        <p className="mt-1.5 text-[10px] text-amber-400/90">Research Stone Spears (Defense) to unlock militia defense & raids.</p>
      )}
    </div>
  );
}