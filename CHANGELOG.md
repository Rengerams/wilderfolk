...
- **More consistent long-running simulation** — the authoritative permanent-removal transition now clears stale child, partner, affair, and pregnancy-parent references from surviving settlers, while adoption and housing retain their existing owners. `tests/familyReferenceCleanup.test.ts` directly covers a dead child being removed from every surviving parent child list. TypeScript, ESLint, the full suite (**82 files / 448 tests**), and the deterministic one-year invariant run (**seed 12345; 0 violations**) passed.
