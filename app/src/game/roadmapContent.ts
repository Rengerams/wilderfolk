...
export const ROADMAP_VERSIONS: RoadmapVersion[] = [
  {
    ...
    features: [
      DONE(
        'Election & leadership — 5-year term ceremony (year-before buildup → 3-day Revelry), incumbent always in race with record score (+8 cap), panel + hints + tutorial synced',
      ),
      DONE('Dead-entity compaction — alive-only entities each tick'),
      DONE('Renderer cache — world.entityByType → RenderSnapshot → updateCachedEntities'),
      PARTIAL('buildingById go-home — drop commute .find()'),
      DONE('Grass render spatial buckets — byType[Grass] + viewport spatial cull'),
      PARTIAL('App tab split + memo @ 300 population'),
      DONE('Spatial grid — grass 56px + mobile 80px; graze→grass, flee/hunt/social→mobile'),
      DONE('Settler count denorm — working/idle on WorldState'),
      DONE('Incremental entityById — update on birth/death only'),
      OPEN('buildingActions scan cleanup — maps instead of entity filters'),
      DONE('Partner id map for relationship lines'),
      OPEN('Particle / floating-text pooling'),
      DONE('Web Worker gameTick — sim off main thread (GameWorkerHost + fallback)'),
      ...
      OPEN('Election Year 5/10 live playtest'),
      OPEN('Footstep / work SFX by surface'),
      OPEN('npm run benchmark:gate — CI wrapper'),
      OPEN('Manual gameplay test — 30+ min end-to-end session, UI/UX + perf notes'),
      OPEN('Audit — code + data consistency review, dead code removal, API surface check'),
      OPEN('Save/load to file — multiple games via download/upload'),
      OPEN('Tick-frequency audit — find systems updated too often (buildings, grids, scans)'),
      OPEN('Ecosystem balance matters — ignoring predator/prey/grass dynamics must have real consequences'),
      OPEN('Wildlife lifespan audit — verify deer/wolf life expectancy and population stability'),
      OPEN('Settler chat balloons — verify JSON dialogue lines show up during interactions'),
      OPEN('Building tick-frequency audit — construction/production once per day, not every tick'),
      OPEN('Remove dead perf code — updatedBuildings clone, workersByBuildingId rebuild, repeated playerHuman filters'),
      OPEN('Multiple save slots — more than one localStorage save'),
    ],
  },
  {
    version: '0.4.2',
